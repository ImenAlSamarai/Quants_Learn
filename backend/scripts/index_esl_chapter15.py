"""
Index Chapter 15 from Elements of Statistical Learning: Random Forests

Creates topics:
1. Random Forests Algorithm
2. Out-of-Bag Error Estimation
3. Feature Importance in Random Forests
4. Random Forests vs Boosting

CRITICAL for quant interviews - Random Forests are used in ~90% of quant shops.
"""
...
class Chapter15Indexer:
    """Indexer for Chapter 15: Random Forests"""
    ...
    def extract_section_range(self, chapter_text: str, start_section: str, end_section: str = None) -> str:
        ...
        Args:
            ...
            start_section: Section to start from (e.g., "15.1")
            end_section: Section to end at (e.g., "15.2"), or None for end of chapter
        """
    ...
    def index_node(
        ...
        subcategory: str = "ensemble_methods",
        ...
        if existing_node:
            ...
            node = Node(
                ...
                color=color,
                icon=icon,
                content_path=f"esl_chapter15_{slug}"
            )
            self.db.add(node)
            self.db.commit()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Index Chapter 15 from ESL")
    ...
    indexer = Chapter15Indexer(args.pdf_path)

    print("=" * 80)
    print("Chapter 15 Indexing: Random Forests")
    print("=" * 80)
    print()

    # Extract chapter
    chapter_data = indexer.extractor.extract_chapter(15)

    if not chapter_data:
        print("Failed to extract Chapter 15!")
        indexer.close()
        return

    chapter_text = chapter_data['text']
    print(f"Chapter 15 extracted: {len(chapter_text):,} characters")
    print()
    ...
    try:
        # Topic 1: Random Forests Algorithm (Section 15.1 and 15.2)
        print("[1/4] Indexing: Random Forests Algorithm")
        section_15_1 = indexer.extract_section_range(chapter_text, "15.1", "15.2")
        section_15_2 = indexer.extract_section_range(chapter_text, "15.2", "15.3")

        rf_algorithm_content = section_15_1 + "\n\n" + section_15_2

        if not rf_algorithm_content or len(rf_algorithm_content) < 1000:
            # Use first third of chapter
            lines = chapter_text.split('\n')
            rf_algorithm_content = '\n'.join(lines[:len(lines)//3])

        indexer.index_node(
            title="Random Forests Algorithm",
            content=rf_algorithm_content,
            description="Bootstrap aggregating, random feature selection, majority voting, forest construction",
            difficulty=4,
            x_pos=-6,
            y_pos=16,
            icon="🌲",
        ...
        # Topic 2: Out-of-Bag Error (Section 15.3 or similar)
        print("[2/4] Indexing: Out-of-Bag Error Estimation")
        section_15_3 = indexer.extract_section_range(chapter_text, "15.3", "15.4")

        if not section_15_3 or len(section_15_3) < 500:
            # Use middle portion
            lines = chapter_text.split('\n')
            start = len(lines) // 3
            end = 2 * len(lines) // 3
        ...
        indexer.index_node(
            title="Out-of-Bag Error Estimation",
            content=section_15_3,
            description="OOB error as validation metric, unbiased error estimation without cross-validation",
            difficulty=4,
            x_pos=-2,
            y_pos=16,
            icon="📊",
        ...
        print("[3/4] Indexing: Feature Importance in Random Forests")
        section_15_4 = indexer.extract_section_range(chapter_text, "15.4", "15.5")

        if not section_15_4 or len(section_15_4) < 500:
            section_15_4 = indexer.extract_section_range(chapter_text, "15.3", "15.4")
        ...
        indexer.index_node(
            ...
            content=section_15_4,
            description="Variable importance measures, permutation importance, Gini importance, feature selection",
            difficulty=4,
            x_pos=2,
            y_pos=16,
            icon="🎯",
        ...
        # Topic 4: Random Forests vs Boosting (comparison content)
        print("[4/4] Indexing: Random Forests vs Boosting")
        # Use end of chapter or combined comparison content
        lines = chapter_text.split('\n')
        comparison_content = '\n'.join(lines[len(lines)//2:])

        indexer.index_node(
            title="Random Forests vs Boosting",
            content=comparison_content,
            description="Ensemble method comparison - parallel vs sequential, variance reduction vs bias reduction",
            difficulty=4,
            x_pos=6,
            y_pos=16,
            icon="⚖️",
        ...
        print("=" * 80)
        print("✓ Chapter 15 indexing completed successfully!")
        print()
        print("Topics added:")
        print("  Random Forests Algorithm")
        print("  Out-of-Bag Error Estimation")
        print("  Feature Importance in Random Forests")
        print("  Random Forests vs Boosting")
        print()
        print("Verify with:")

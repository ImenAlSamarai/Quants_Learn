"""
Index Chapter 12 from Deep Learning: Foundations and Concepts - Transformers

Creates topics:
1. Attention Mechanisms
2. Self-Attention and Multi-Head Attention
3. Transformer Architecture
4. Transformer Language Models (BERT, GPT)

CRITICAL for quant finance - Transformers for NLP (news, earnings calls), time series forecasting.
"""
...
class Chapter12Indexer:
    """Indexer for Chapter 12: Transformers"""
    ...
    def index_node(
        ...
        if existing_node:
            ...
            node = Node(
                ...
                color=color,
                icon=icon,
                content_path=f"dl_chapter12_{slug}"
            )
            self.db.add(node)
            self.db.commit()
        ...
        for i, chunk_text in enumerate(chunks):
            chunk_data.append({
                ...
                'metadata': {
                    ...
                    'source': 'Deep Learning: Foundations and Concepts, Chapter 12',
                }
            })
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Index Chapter 12 from DL book")
    ...
    indexer = Chapter12Indexer(args.pdf_path)

    print("=" * 80)
    print("Chapter 12 Indexing: Transformers")
    print("=" * 80)
    print()

    chapter_data = indexer.extractor.extract_chapter(12)

    if not chapter_data:
        print("Failed to extract Chapter 12!")
        indexer.close()
        return

    chapter_text = chapter_data['text']
    print(f"Chapter 12 extracted: {len(chapter_text):,} characters")
    ...
    try:
        ...
        # Topic 1: Attention Mechanisms (first 25%)
        print("[1/4] Indexing: Attention Mechanisms")
        section_1 = '\n'.join(lines[:total_lines//4])

        indexer.index_node(
            title="Attention Mechanisms",
            content=section_1,
            description="Attention scores, query-key-value, attention weights, context vectors, alignment",
            difficulty=4,
            x_pos=-6,
            y_pos=28,
            icon="👁️",
        ...
        # Topic 2: Self-Attention and Multi-Head Attention
        print("[2/4] Indexing: Self-Attention and Multi-Head Attention")
        section_2 = '\n'.join(lines[total_lines//4:total_lines//2])

        indexer.index_node(
            title="Self-Attention and Multi-Head Attention",
            content=section_2,
            description="Scaled dot-product attention, multi-head attention, parallel attention heads, Q/K/V matrices",
            difficulty=5,
            x_pos=-2,
            y_pos=28,
            icon="🔍",
        ...
        print("[3/4] Indexing: Transformer Architecture")
        section_3 = '\n'.join(lines[total_lines//2:3*total_lines//4])

        indexer.index_node(
            title="Transformer Architecture",
            content=section_3,
            description="Encoder-decoder structure, positional encoding, feed-forward layers, layer normalization",
            difficulty=5,
            x_pos=2,
            y_pos=28,
            icon="🏛️",
        ...
        # Topic 4: Transformer Language Models
        print("[4/4] Indexing: Transformer Language Models (BERT, GPT)")
        section_4 = '\n'.join(lines[3*total_lines//4:])

        indexer.index_node(
            title="Transformer Language Models (BERT, GPT)",
            content=section_4,
            description="BERT, GPT, pre-training, fine-tuning, masked language modeling, autoregressive models",
            difficulty=5,
            x_pos=6,
            y_pos=28,
            icon="💬",
        ...
        print("=" * 80)
        print("✓ Chapter 12 indexing completed successfully!")
        print()
        print("Topics added:")
        ...
        print()
        print("Verify with:")
        print("  python scripts/check_indexed_content.py | grep -i 'transform\\|attention'")

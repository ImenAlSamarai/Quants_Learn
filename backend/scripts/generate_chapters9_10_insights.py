"""
Extract and generate practitioner insights for Chapters 9-10 topics (Trees and Boosting)
"""
...
"""
    ...
    def generate_insights_for_topic(self, node: Node, chapter_num: int):
        ...
        if not biblio_notes or len(biblio_notes) < 100:
            print(f"  No bibliographic notes for Chapter {chapter_num}, using generic prompt")
            # Fall back to generating from topic name
            biblio_notes = f"This is a topic from Chapter {chapter_num} of Elements of Statistical Learning about {node.title}."
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate insights for Chapters 9-10 topics")
    ...
    print("=" * 80)
    print("Chapters 9-10 Insights Generation: Trees and Boosting")
    print("=" * 80)

    # Chapter 9 topics and Chapter 10 topics combined
    topics_with_chapters = [
        ("Decision Trees (CART)", 9),
        ("Regression Trees", 9),
        ("Tree Pruning Methods", 9),
        ("AdaBoost Algorithm", 10),
        ("Gradient Boosting Machines", 10),
        ("Boosting vs Bagging", 10),
    ]

    db = SessionLocal()

    try:
        for topic_title, chapter_num in topics_with_chapters:
        ...
        print("\n" + "=" * 80)
        print("✓ Chapters 9-10 insights generation completed!")
